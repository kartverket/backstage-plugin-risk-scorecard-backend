...
enum class GithubStatus {
    NotFound, Unauthorized, ContentIsEmpty, Success, RequestResponseBodyError, ResponseBodyTooLargeForWebClientError, InternalError,
}
...
data class GithubWriteToFilePayload(
    ...
    fun toContentBody(): String = when (author) {
        null -> when (sha) {
            null -> "{\"message\":\"$message\", \"content\":\"$content\", \"branch\": \"$branchName\"}"

            else -> "{\"message\":\"$message\", \"content\":\"$content\", \"branch\": \"$branchName\", \"sha\":\"$sha\"}"
        }
...
class GithubConnector(
    ...
    fun fetchSopsConfigFromDefaultBranch(
        ...
        val sopsConfig = getGithubResponse(
            githubHelper.uriToFindSopsConfig(repositoryOwner, repositoryName),
            githubAccessToken.value,
        ).bodyToMono<FileContentDTO>().block()?.content?.decodeBase64()
        return when (sopsConfig) {
            null -> throw SopsConfigFetchException(
                message = "Fetch of sops config responded with 200 OK but file contents was null",
                riScId = "",
    ...
    suspend fun fetchPublishedRiSc(
        ...
        val fileContent = getGithubResponseSuspend(
    ...
    suspend fun fetchDraftedRiScContent(
        ...
        val fileContent = getGithubResponseSuspend(
            githubHelper.uriToFindRiScOnDraftBranch(owner, repository, id), accessToken
        ).decodedFileContentSuspend()
        when (fileContent) {
            null -> GithubContentResponse(null, GithubStatus.ContentIsEmpty)
            else -> GithubContentResponse(fileContent, GithubStatus.Success)
    ...
    private suspend fun fetchRiScIdentifiersSentForApproval(
        ...
        val response = getGithubResponseSuspend(
            githubHelper.uriToFetchAllPullRequests(owner, repository), accessToken
    ...
    internal fun updateOrCreateDraft(
        ...
        // through updating.
        val commitMessage = if (latestShaForDraft == null) {
        ...
        val riScApprovalPRStatus = runBlocking {
            ...
            val commitMessages = latestCommitTimestamp?.let { it ->
            ...
            if (!requiresNewApproval && !prExists && !commitsAheadOfMainRequiresApproval) {
                val pullRequest = createPullRequestForRiSc(
                    owner,
                    repository,
                    riScId,
                    requiresNewApproval,
    ...
    private fun closePullRequestForRiSc(
        ...
        riScId: String,
        accessToken: String,
    ): String? = fetchAllPullRequests(owner, repository, accessToken).find { it.head.ref == riScId }?.let {
        try {
            closePullRequest(
    ...
    private fun getSHAForExistingRiScDraftOrNull(
        ...
        getGithubResponse(
            githubHelper.uriToFindRiScOnDraftBranch(owner, repository, riScId), accessToken
        ).shaResponseDTO()
    } catch (e: Exception) {
        null
    }
    ...
    fun fetchAllPullRequests(
        ...
        repository: String,
        accessToken: String,
    ): List<GithubPullRequestObject> = try {
        getGithubResponse(
            githubHelper.uriToFetchAllPullRequests(owner, repository), accessToken
    ...
    private fun fetchLatestCommitTimestampOnDefault(
        ...
        getGithubResponse(
            githubHelper.uriToFetchCommitOnMain(owner, repository, riScId), accessToken
    ...
    fun createPullRequestForSopsConfig(
        ...
        gitHubAccessToken: GithubAccessToken,
        defaultBranch: String,
    ): GithubPullRequestObject? = postNewPullRequestToGithub(
        uri = githubHelper.uriToCreatePullRequest(owner, repository),
        accessToken = gitHubAccessToken.value,
        pullRequestPayload = GithubCreateNewPullRequestPayload(
            "Update SOPS configuration",
            "This pull request updates the SOPS configuration that is needed to encrypt and decrypt RiSc's in [Risk Scorecard in Kartverket.dev](https://kartverket.dev/catalog/default/component/$repository/risc). Merge this PR in order to use the new SOPS configuration in the [Risk Scorecard plugin](https://kartverket.dev/catalog/default/component/$repository/risc).",
            owner,
            sopsId,
            defaultBranch,
        ),
    ...
    fun putFileRequestToGithub(
        ...
        val payload = GithubWriteToFilePayload(
            message = message,
            content = content,
        ...
        return try {
            webClient.put().uri(githubHelper.repositoryContentsUri(repositoryOwner, repositoryName, filePath, branch))
                .header("Accept", "application/vnd.github+json")
                .header("Authorization", "token ${gitHubAccessToken.value}")
                .header("X-GitHub-Api-Version", "2022-11-28").header("Content-Type", "application/json")
                .body(Mono.just(payload.toContentBody()), String::class.java).retrieve()
    ...
    private suspend fun getGithubResponseSuspend(
        ...
        return webClient.get().uri(uri).header("Accept", "application/vnd.github.json")
    ...
    private fun getGithubResponse(
        uri: String,
        accessToken: String,
    ): ResponseSpec = webClient.get().uri(uri).header("Accept", "application/vnd.github.json")
        .header("Authorization", "token $accessToken").header("X-GitHub-Api-Version", "2022-11-28").retrieve()
    ...
    private fun ResponseSpec.toPullRequestFilesDTO(): List<PullRequestFileObject>? =
        this.bodyToMono<List<PullRequestFileObject>>().block()
    ...
    private fun List<GithubPullRequestObject>.riScIdentifiersSentForApproval(): List<RiScIdentifier> = this.map {
        RiScIdentifier(
            it.head.ref.split("/").last(),
    ...
    private fun ResponseSpec.toRepositoryBranchDTO(): List<RepositoryBranchDTO>? =
        this.bodyToMono<List<RepositoryBranchDTO>>().block()

    private fun ResponseSpec.toFileContentsDTO(): List<FileContentsDTO>? =
        this.bodyToMono<List<FileContentsDTO>>().block()
    ...
    ) = RestClient.builder().baseUrl("https://api.github.com/repos")
        .defaultHeader(HttpHeaders.AUTHORIZATION, "token $gitHubAccessToken")
        .defaultHeader(HttpHeaders.ACCEPT, "application/vnd.github.json")
        .defaultHeader("X-GitHub-Api-Version", "2022-11-28").build().get().uri { uriBuilder ->
            uriBuilder.path(uri).build()
        }.retrieve().body(object : ParameterizedTypeReference<RepositoryDTO>() {})!!
    ...
    fun fetchRepositoryInfo(
        ...
        val repositoryDTO = fetchRepositoryInfo(
    ...
    ): List<GithubPullRequestObject> = getGithubResponse(
        githubHelper.uriToFetchAllPullRequests(repositoryOwner, repositoryName),
        gitHubAccessToken.value,
    ).toPullRequestResponseDTOs().filter {
        it.head.ref in branches && it.base.ref == defaultBranch
    }
    ...
    fun fetchAllRiScsOnDefaultBranch(
        ...
        return fileContentPaths.filter { it.name.endsWith("$filenamePostfix.json") || it.name.endsWith("$filenamePostfix.yaml") }
            .associateWith {
                getGithubResponse(
                    githubHelper.repositoryContentsUri(repositoryOwner, repositoryName, it.path),
                    gitHubAccessToken.value,
                ).toFileContentDTO()?.content?.decodeBase64() ?: throw GitHubFetchException(
